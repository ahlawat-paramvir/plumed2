...
//			SwitchingFunction switchingFunction1;
			...
			public:
			explicit dsfTwo(const ActionOptions&);
			~dsfTwo();
			// active methods:
		...
		PLUMED_REGISTER_ACTION(dsfTwo,"DSFTWO")

			void dsfTwo::registerKeywords( Keywords& keys ){
...
//				keys.add("optional","SWITCH1","Switching function 1.");
		...
		dsfTwo::dsfTwo(const ActionOptions&ao):
			...
			firsttime(true)
		{
			...
			log.printf("  between four groups of %u, %u, %u and %u atoms\n",static_cast<unsigned>(center_lista.size()),static_cast<unsigned>(around_species1_lista.size()));
				...
				for(unsigned int j=0;j<neighbors.size();j+=1) {   
					...
					if (atomsToRequest[i].serial()!=atomsToRequest[neighbors[j]].serial()) { 
						...
						if ( (neighbors[j]>=center_lista.size()) && (neighbors[j]<(center_lista.size()+around_species1_lista.size())) ) {
							...
							Vector dd = dij*((window*(QR1*std::cos(QR1)-std::sin(QR1))/R_ij+d_window*std::sin(QR1))/QR1/R_ij)/nn;
							deriv1[i] -= dd;
		...
		//		CVvalue += CVvalue1;
				...
				for(unsigned int j=0;j<getNumberOfAtoms();j+=1) {   
					for(unsigned int k=0;k<3;k+=1) {  
						//deriv[j][k] += deriv1[j][k]*dfunc1;
						deriv[j][k] += deriv1[j][k];
					}
				}
				for(unsigned int j=0;j<3;j+=1) {   
					for(unsigned int k=0;k<3;k+=1) {
					//	virial[j][k] += virial1[j][k]*dfunc1;
						virial[j][k] += virial1[j][k];
					}
				}
			}
			if(!serial){
